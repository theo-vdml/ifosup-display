...
const buildSeedRules = (): RecurrenceRule[] => {
    if (!props.rooms.length || !props.courses.length) {
        return [];
    }

    const roomA = props.rooms[0];
    const roomB = props.rooms[1] ?? props.rooms[0];
    const roomC = props.rooms[2] ?? props.rooms[0];
    const courseA = props.courses[0];
    const courseB = props.courses[1] ?? props.courses[0];
    const courseC = props.courses[2] ?? props.courses[0];

    return [
        { id: 1, day: 'monday', period: 'morning', startDate: '2026-03-10', endDate: '2026-07-20', room: roomA, course: courseA },
        { id: 2, day: 'monday', period: 'afternoon', startDate: '2026-03-15', endDate: '2026-06-25', room: roomB, course: courseB },
        { id: 3, day: 'tuesday', period: 'morning', startDate: '2026-03-05', endDate: '2026-09-01', room: roomC, course: courseC },
        { id: 4, day: 'wednesday', period: 'evening', startDate: '2026-04-01', endDate: '2026-08-15', room: roomA, course: courseB },
        { id: 5, day: 'friday', period: 'afternoon', startDate: '2026-03-20', endDate: '2026-07-30', room: roomB, course: courseA },
    ];
};

const draft = reactive<RecurrenceDraft>(createDraft());
const rules = ref<RecurrenceRule[]>(buildSeedRules());
...
const nextId = computed(() => {
    return rules.value.length > 0 ? Math.max(...rules.value.map((rule) => rule.id)) + 1 : 1;
});
...
const saveRule = () => {
    ...
    const payload: Omit<RecurrenceRule, 'id'> = {
        day: draft.day,
        period: draft.period,
        startDate: draft.startDate,
        endDate: draft.endDate,
        room: draft.room as Room,
        course: draft.course as Course,
    };

    if (isEditing.value) {
        rules.value = rules.value.map((rule) => {
            return rule.id === editingId.value ? { ...rule, ...payload } : rule;
        });
    }
    else {
        rules.value = [{ id: nextId.value, ...payload }, ...rules.value];
    }
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="px-4 py-6 w-full max-w-5xl mx-auto">
            <div class="flex flex-col space-y-6">
                ...
                <div v-if="!hasCatalogData" class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Impossible de creer une recurrence: la base doit contenir au moins un local et un cours.
                </div>

                <div v-if="!rules.length" class="aspect-video w-full rounded-xl border-dashed border-secondary border-2">
                    ...
                    <div class="rounded-xl border border-sidebar-border/60 bg-white dark:bg-zinc-900/50 p-3 sm:p-4">
                        <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                            ...
                            <p class="text-xs text-muted-foreground lg:text-right">
                                ...
                                <span class="hidden sm:inline"> · {{ filteredRules.length }} correspondance(s) globale(s)</span>
                            </p>
                        </div>
                    </div>

                    <div class="rounded-xl border border-sidebar-border/60 bg-white dark:bg-zinc-900/50 overflow-hidden">
                        <div class="border-b px-3 py-3 sm:px-4">
                            <div class="flex gap-2 overflow-x-auto pb-1">
                                ...
                                >
                                    {{ day.label }}
                                    <Badge variant="secondary" class="ml-2 px-1.5 py-0">{{ getDayCount(day.key) }}</Badge>
                                </Button>
                            </div>
                        </div>
                        ...
                        <div class="border-b px-3 py-3 sm:px-4">
                            <div class="flex gap-2 overflow-x-auto pb-1">
                                ...
                                >
                                    {{ period.label }}
                                    <Badge variant="secondary" class="ml-2 px-1.5 py-0">{{ periodCounters[period.key] }}</Badge>
                                </Button>
                            </div>
                        </div>
                        ...
                        <div v-else class="overflow-x-auto">
                            <table class="w-full min-w-225 text-left">
                                <thead class="bg-zinc-50/60 dark:bg-zinc-900/50 border-b">
                                    <tr>
                                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Creneau</th>
                                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Plage</th>
                                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Cours</th>
                                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Local</th>
                                        <th class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500 text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y">
                                    <tr v-for="rule in visibleRules" :key="rule.id" class="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                                        <td class="px-4 py-3">
                                            <div class="space-y-0.5">
                                                <p class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ dayLabel(rule.day) }}</p>
                                                <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ periodLabel(rule.period) }}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="space-y-0.5 min-w-56">
                                                <p class="text-sm text-zinc-900 dark:text-zinc-100">Du {{ formatRuleRange(rule).from }}</p>
                                                <p class="text-sm text-zinc-900 dark:text-zinc-100">Au {{ formatRuleRange(rule).to }}</p>
                                                <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ formatRuleOccurrencesLabel(rule) }}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="space-y-0.5">
                                                <p class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ rule.course.code }}</p>
                                                <p class="text-sm text-zinc-700 dark:text-zinc-300">{{ rule.course.name }}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ rule.room.name }}</p>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <Button size="icon-sm" variant="outline" @click="openEditDrawer(rule)" title="Modifier" aria-label="Modifier">
                                                    <PencilLine class="size-3.5" />
                                                </Button>
                                                <Button size="icon-sm" variant="outline" class="text-red-600" @click="deleteRule(rule.id)" title="Supprimer" aria-label="Supprimer">
                ...
                <div class="space-y-5 px-4 py-4">
                    <div class="grid gap-2">
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                            <Button
                                ...
                                :key="day.key"
                                type="button"
                                :variant="draft.day === day.key ? 'default' : 'outline'"
                                class="justify-start"
                    ...
                    <div class="grid gap-2">
                        ...
                        <SchedulerDateRangePicker
                            :from-date="draft.startDate"
                            :to-date="draft.endDate"
                    ...
                    <div class="grid gap-2">
                        ...
                        <Combobox
                            ...
                            :options="props.rooms"
                            value-key="id"
                            :display-function="displayRoomLabel"
                            placeholder="Selectionner un local"
                            nullable
                        />
                    </div>
                    ...
                    <div class="grid gap-2">
                        ...
                        <Combobox
                            ...
                            value-key="id"
                            :display-function="displayCourseLabel"
